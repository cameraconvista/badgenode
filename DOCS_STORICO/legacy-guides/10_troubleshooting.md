...
**Versione**: 5.2 • **Data**: 2026-04-15 • **Stato**: Allineato con audit schema read-only
...
**Stato audit 2026-04-15**: i campi `email`, `telefono`, `ore_contrattuali`, `note` risultano presenti nel DB reale. Questa azione manuale va quindi considerata storica e non più assunta come pending senza verifica aggiuntiva del contesto.

**Nota audit aggiuntiva 2026-04-15**:
- il DB reale non mostra `utenti.id`
- `utenti.pin` resta la PK reale
- l'azione corretta lato runtime continua a essere l'uso di query schema-agnostiche quando serve compatibilità
...
### Stato reale Supabase emerso dall'audit read-only

`Certo`
- in `public` esistono solo `utenti`, `timbrature`, `ex_dipendenti`
- in `public` non esistono `view` né `materialized view`
- `v_turni_giornalieri` non risulta presente nel DB reale
- `public.timbrature` non ha trigger attivi
- `timbrature` espone idempotenza reale via `client_event_id` univoco, non via `device_id/client_seq`
- `utenti` e `timbrature` hanno RLS attivo; `ex_dipendenti` no
- `service_role` bypassa RLS; `anon` e `authenticated` no
- nel perimetro `public` esistono solo queste funzioni: `enforce_alternanza_fn`, `enforce_alternanza_simple_fn`, `generate_timbrature_report`, `insert_timbro_v2`
- le funzioni `public` sopra hanno grant `EXECUTE` anche per `PUBLIC`, `anon`, `authenticated`, `service_role`

`Legacy / disallineato`
- `insert_timbro_v2` è scrivente e invocabile, ma il commento interno che presume trigger attivi è disallineato rispetto allo stato reale
- `enforce_alternanza_fn` e `enforce_alternanza_simple_fn` esistono ma non sono operative perché non agganciate a trigger attivi
- la documentazione storica che parla di view operative o trigger DB attivi non descrive il comportamento runtime attuale

`Da verificare`
- eventuali oggetti fuori `public`, non inclusi in questo audit
...
- `GET /api/storico` va considerato allineato al fallback applicativo su tabella `timbrature`; `v_turni_giornalieri` non esiste nel DB reale.
