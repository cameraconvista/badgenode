...
console.log(
  ...
  process.env.VITE_SUPABASE_ANON_KEY ? 'Presente' : 'Mancante'
);
console.log(
...
async function main() {
  ...
  try {
    ...
    console.log('\n📊 Test lettura utenti...');
    const { data: utenti, error: utentiError } = await supabase.from('utenti').select('*').limit(5);
    ...
    try {
      const { data: rpcTest, error: rpcError } = await supabase.rpc('insert_timbro_v2', {
        p_pin: 1,
        p_tipo: 'entrata',
      });
