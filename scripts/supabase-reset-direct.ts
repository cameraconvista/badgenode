...
async function main() {
  ...
  try {
    ...
    console.log('\n🌱 Seed utenti...');
    const { data: seedData, error: seedError } = await supabase.from('utenti').upsert(
      [
    ...
    try {
      const { data: entrata, error: entrataError } = await supabase.rpc('insert_timbro_v2', {
        p_pin: 1,
        p_tipo: 'entrata',
      });
    ...
    try {
      const { data: uscita, error: uscitaError } = await supabase.rpc('insert_timbro_v2', {
        p_pin: 1,
        p_tipo: 'uscita',
      });
    ...
    try {
      const { data: pinError, error: pinErrorResult } = await supabase.rpc('insert_timbro_v2', {
        p_pin: 99,
        p_tipo: 'entrata',
      });
