  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      FEATURE_LOGGER_ADAPTER
        ? log.info({ traceId, pin: pinNum, route: 'pin:validate' }, 'starting')
    ...
    if (qErr) {
      ...
      if (qCode === 'PGRST116' || /no rows|results contain 0 rows|row not found/i.test(String(qMsg))) {
        if (process.env.NODE_ENV === 'development') {
          FEATURE_LOGGER_ADAPTER
            ? log.info({ traceId, pin: pinNum, route: 'pin:validate' }, 'not_found')
      ...
      if (process.env.NODE_ENV === 'development') {
        FEATURE_LOGGER_ADAPTER
          ? log.error({ traceId, pin: pinNum, error: qMsg || qErr, route: 'pin:validate' }, 'query_error')
