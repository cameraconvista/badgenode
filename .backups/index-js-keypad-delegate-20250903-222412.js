...
// === Robust keypad/admin delegate (idempotent) ===
(function robustKeypadDelegate(){
  // esponi comunque le funzioni in globale (senza rompere se già fatte)
  try{ if (typeof clickNumero==='function')      window.clickNumero      = clickNumero; }catch(_){}
  try{ if (typeof cancellaUltimo==='function')   window.cancellaUltimo   = cancellaUltimo; }catch(_){}
  try{
    if (typeof apriModalAdmin==='function')      window.apriModalAdmin   = apriModalAdmin;
    else if (typeof apriModalaAdmin==='function') window.apriModalAdmin  = apriModalaAdmin;
  }catch(_){}
  try{ if (typeof verificaPinAdmin==='function') window.verificaPinAdmin = verificaPinAdmin; }catch(_){}
  try{ if (typeof chiudiModalAdmin==='function') window.chiudiModalAdmin = chiudiModalAdmin; }catch(_){}

  const bind = () => {
    const root = document;
    if (root.__robustKeypadBound) return;
    root.__robustKeypadBound = true;

    root.addEventListener('click', (ev) => {
      // prendi QUALSIASI cosa cliccabile (anche a/div/li/span) e risali
      const el = ev.target.closest('[data-key],[data-action],[data-role],button,a,[role="button"],.btn,.key,.tasto,.digit,li,div,span');
      if (!el) return;

      // estrai etichetta/azione dalla fonte migliore
      const label =
        (el.dataset && (el.dataset.key || el.dataset.digit || el.dataset.action || el.dataset.role)) ||
        (el.getAttribute && (el.getAttribute('data-key') || el.getAttribute('aria-label'))) ||
        (el.innerText || el.textContent || '');

      const txt = String(label).trim().toLowerCase();

      // previeni navigazioni #/a e doppi handler
      if (el.tagName === 'A' || el.getAttribute?.('href') === '#') { ev.preventDefault(); }
      // se un altro delegato ha già gestito, esci
      if (ev.__keyHandled) return;

      // 1) Cifre 0–9 (anche se il testo contiene spazi/icone)
      const digitMatch = txt.match(/(^|\s)([0-9])($|\s)/);
      if (digitMatch && typeof window.clickNumero === 'function') {
        ev.__keyHandled = true;
        ev.stopPropagation(); ev.stopImmediatePropagation();
        window.clickNumero(parseInt(digitMatch[2], 10));
        return;
      }

      // 2) Cancella (C, "canc", freccia ←)
      if ((/^(c|canc|←)$/.test(txt) || txt.includes('canc')) && typeof window.cancellaUltimo === 'function') {
        ev.__keyHandled = true;
        ev.stopPropagation(); ev.stopImmediatePropagation();
      ...
      // 3) Ingranaggio Admin (icona ⚙, aria/data "admin", classi tipiche)
      const raw = (label || '').toString();
      const aria = (el.getAttribute?.('aria-label') || '').toLowerCase();
      const classes = (el.className || '').toString().toLowerCase();
      if (
        raw.includes('⚙') || txt.includes('admin') || aria.includes('admin') || classes.includes('gear') || classes.includes('admin')
      ) {
        if (typeof window.apriModalAdmin === 'function') {
          ev.__keyHandled = true;
          ev.stopPropagation(); ev.stopImmediatePropagation();
          window.apriModalAdmin();
        }
      }
    }, true);
  };
