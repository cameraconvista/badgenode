...
export default function ActionButtons({
  ...
  return (
    <div className={`w-full ${className}`}>
      <div className="grid grid-cols-2 gap-2 sm:gap-3 max-w-[260px] sm:max-w-[280px] mx-auto px-1 sm:px-2">
        <button
          data-testid="button-entrata"
          onClick={onEntrata}
          disabled={disabled || disabledEntrata}
          className="
            min-h-[52px] sm:min-h-[56px] px-3 sm:px-6
            rounded-full
            font-semibold text-sm sm:text-base md:text-lg tracking-wide uppercase
