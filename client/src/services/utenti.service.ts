...
export class UtentiService {
  ...
  static async getUtenti(traceId?: string): Promise<Utente[]> {
    try {
      logTimbraturaDiag('utenti.fetch_start', {
        traceId,
        source: 'utenti-service',
      });
      const requestOptions = traceId
        ? { headers: { 'x-badgenode-trace': traceId } }
        : undefined;
      const response = await safeFetchJson<DbUtente[]>('/api/utenti', requestOptions);
  ...
  static async getUtenteByPin(pin: number, traceId?: string): Promise<Utente | null> {
    try {
      logTimbraturaDiag('utenti.by_pin_start', {
        traceId,
        pin,
        source: 'utenti-service',
      });
      const requestOptions = traceId
        ? { headers: { 'x-badgenode-trace': traceId } }
        : undefined;
      const response = await safeFetchJson<DbUtente>(`/api/utenti/${pin}`, requestOptions);
      ...
      if (isError(response)) {
        logTimbraturaDiag('utenti.by_pin_result', {
          ...
          source: 'utenti-service',
          found: false,
          error: response.code || response.error || 'lookup-failed',
        });
        return null;
      }
