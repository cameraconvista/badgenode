...
export function initMonitoring(): void {
  ...
  if (!isProduction || !isEnabled || !hasDSN) {
    log.info('[Monitoring] Disabled (dev mode or feature flag OFF)');
    return;
  }
  ...
  try {
    ...
    */
    ...
    log.info('[Monitoring] Ready (Sentry stub - install @sentry/node to activate)');
  } catch (error) {
...
export function captureError(error: unknown, context?: Record<string, unknown>): void {
  ...
  if (isEnabled) {
    ...
    log.error('[Monitoring] Error captured:', { error, context });
  } else {
    log.error('[Monitoring] Error (not sent):', error);
  }
}
...
export function captureMessage(
  ...
  if (isEnabled) {
    ...
    log.info('[Monitoring] Message captured:', { message, level, context });
  }
}
...
export async function traceTransaction<T>(
  ...
  try {
    ...
    log.info('[Monitoring] Transaction:', { name, operation, duration });
...
export function setUserContext(userId: string, metadata?: Record<string, unknown>): void {
  ...
  if (isEnabled) {
    ...
    log.info('[Monitoring] User context set:', { userId, metadata });
  }
}
...
export function clearUserContext(): void {
  ...
  if (isEnabled) {
    // Sentry.setUser(null);
    log.info('[Monitoring] User context cleared');
  }
}
...
export function addBreadcrumb(
  ...
  if (isEnabled) {
    ...
    log.debug('[Monitoring] Breadcrumb:', { category, message, data });
  }
}
