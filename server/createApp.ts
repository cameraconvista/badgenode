...
export function createApp() {
  ...
  app.use((req, res, next) => {
    log.http('[REQ]', req.method, req.url);
    next();
  });
...
export async function setupStaticFiles(app: express.Express, server?: http.Server) {
  ...
  if (app.get('env') === 'development') {
    if (!server) {
      throw new Error('HTTP server required in development mode');
    }
